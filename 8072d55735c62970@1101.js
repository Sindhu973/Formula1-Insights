...
md`### Question 4: 
How does the fastest lap time compare among the race circuits across all years?`
)}
...
function _circuitsWithGrid(d3,q4final,gridPositions){return(
d3.zip(q4final, gridPositions)
    .map(([data, position]) => ({...data, ...position}))
)}

function _q4extent(d3,q4final){return(
d3.extent(q4final, d => d.fastestLapTime)
)}

function _q4finalspeedometer(d3,numCols,numRows,q4extent,circuitsWithGrid,lightgray)
{
  ...
  g.append('text')
      ...
      .attr('x', visWidth / 2)
      .attr('y', -margin.top + 5)
      .text('Fastest Lap Times (seconds) for Circuits');
...
md`### Explanation:
We have used speedometer charts to visualize the fastest lap for all circuits and have presented them in increasing order of the time.
We can see that the fastest lap time for Bahrain International Circuit was about 56 seconds whereas for Spa-Francorchamps it is about 106 seconds.

This metric also helps us understand the length of each lap for each circuit. A circuit with a lower lap time implies that its lap distance would be less.`
)}
...
function _q5Data(psKeys,q5DataGrouped)
{
  let temp = []
  psKeys.forEach(x=>{
    let constData = q5DataGrouped.get(x)
    let data = []
    constData.forEach(y=>{
      let td = {
...
d3.min(q5Data, constructor => d3.max(constructor.data, d => d.year))
)}
...
function _q6constructors(drivList,q6Const)
{
  let tempSet = new Set()
...
function _95(Swatches,d3,meanPositions){return(
Swatches(d3.scaleOrdinal(meanPositions,d3.schemeTableau10))
)}

function _96(width,d3,q6constructors,drivList,groupedData3,driverConstructor,color2)
{
...
function _101(Swatches,color7){return(
Swatches(color7,{})
)}
...
function _getConstRecentName(){return(
function getConstRecentName(name) {
  const map = new Map()

  map.set('Toro Rosso', 'AlphaTauri')
  map.set('Sauber', 'Alfa Romeo')
  map.set('Force India', 'Aston Martin')
  map.set('Racing Point', 'Aston Martin')
  map.set('Renault', 'Alpine F1 Team')
  if(map.has(name))
    return map.get(name)
  return name
}
...
function _top_10_constructors_new(top_10_constructors,getConstructorName)
{
  let res = []
  top_10_constructors.forEach( d=> {
    res.push(getConstructorName(d))
  })
  return res
}


function _top_20_drivers_new(top_20_drivers,getDriverName)
{
  let res = []
  top_20_drivers.forEach( d=> {
    res.push(getDriverName(d))
  })
  return res
}


function _viz_data11_new(viz_data11,getDriverName,getConstructorName)
{
  let res = []
  viz_data11.forEach( d=> {
    res.push({Drivers:getDriverName(d.Drivers),Constructors:getConstructorName(d.Constructors), points:d.points})
  })
  return res    
}


function _125(legend,color11){return(
legend({
  color: color11,
  width: 500,
  ticks: 5,
...
export default function define(runtime, observer) {
  ...
  main.import("driverMapRev", child5);
  ...
  main.variable(observer("circuitsWithGrid")).define("circuitsWithGrid", ["d3","q4final","gridPositions"], _circuitsWithGrid);
  main.variable(observer("q4extent")).define("q4extent", ["d3","q4final"], _q4extent);
  main.variable(observer("q4finalspeedometer")).define("q4finalspeedometer", ["d3","numCols","numRows","q4extent","circuitsWithGrid","lightgray"], _q4finalspeedometer);
  main.variable(observer()).define(["md"], _64);
  main.variable(observer()).define(["md"], _65);
  main.variable(observer("q5DataGrouped")).define("q5DataGrouped", ["d3","q5_pitStopsData"], _q5DataGrouped);
  main.variable(observer("q5Data")).define("q5Data", ["psKeys","q5DataGrouped"], _q5Data);
  ...
  main.variable(observer("q6constructors")).define("q6constructors", ["drivList","q6Const"], _q6constructors);
  ...
  main.variable(observer()).define(["Swatches","d3","meanPositions"], _95);
  main.variable(observer()).define(["width","d3","q6constructors","drivList","groupedData3","driverConstructor","color2"], _96);
  ...
  main.variable(observer()).define(["Swatches","color7"], _101);
  ...
  main.variable(observer("getConstRecentName")).define("getConstRecentName", _getConstRecentName);
  main.variable(observer("top_10_constructors_new")).define("top_10_constructors_new", ["top_10_constructors","getConstructorName"], _top_10_constructors_new);
  main.variable(observer("top_20_drivers_new")).define("top_20_drivers_new", ["top_20_drivers","getDriverName"], _top_20_drivers_new);
  main.variable(observer("viz_data11_new")).define("viz_data11_new", ["viz_data11","getDriverName","getConstructorName"], _viz_data11_new);
  main.variable(observer()).define(["legend","color11"], _125);
