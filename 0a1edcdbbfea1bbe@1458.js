...
function _86(Inputs,q5_pitStopsData){return(
...
function _97(Inputs,q6final){return(
...
function _races2021(d3,races){return(
d3.group(
  races.filter(d=>d.year==2021).map(d=>({'raceId':d.raceId,'raceName':d.name})).sort((a,b) => a.raceId - b.raceId),
  //g=>g.raceId,
  d=>d.raceId
  )
)}
...
function _q7data(drivers2021,driverMap,accumulate,raceResults2021)
{
  ...
  drivers2021.forEach( dr=> {
    pointsProg.push({driver:driverMap.get(dr),cumulativePoints:accumulate(raceResults2021.filter(d=>d.driverId==dr).map(d=>({'raceId':d.raceId,'raceName':d.raceName,'points':d.points})).sort((a,b) => a.raceId - b.raceId).map(d=>d.points))})  
  })
  return  pointsProg
}


function _q7datafinal(q7data)
{
  let res = []
  q7data.forEach( d=> {
    let rPt = []
    let race = 1
    d.cumulativePoints.forEach( pt => {
      rPt.push({race:race,points:pt})
      race = race + 1
    })
    res.push({driver:d.driver,points:rPt})
  })
  return res
}


function _q7VizData(q7data)
{
  let res = []
  q7data.forEach( d=> {
    ...
    let race = 1
    d.cumulativePoints.forEach( pt => {
      res.push({driver:d.driver,race:race,points:pt})
      race = race + 1
    })
...
function _const2021(raceResults2021){return(
new Set(raceResults2021.map(d=>d.constructorId))
)}

function _q7dataconst(const2021,q5constructorMap,accumulate,raceResults2021)
{
  let pointsProg = []
  const2021.forEach( c=> {
    pointsProg.push({constructor:q5constructorMap.get(c),cumulativePoints:accumulate(raceResults2021.filter(d=>d.constructorId==c).map(d=>({'raceId':d.raceId,'points':d.points})).sort((a,b) => a.raceId - b.raceId).map(d=>d.points))})  
  })
  return  pointsProg
}


function _111(Inputs,q7data){return(
Inputs.table(q7data)
)}
...
md`Choose Driver/Constructor option`
)}
...
d3.rollups(
  ...
  g=>g.length,
  d=>d.driverId
  ).map(d=>({'map':driverMap.get(d[0]),'value':d[1]}))
)}

function _120(Inputs,wins){return(
Inputs.table(wins)
)}
...
function _123(Inputs,podiums){return(
...
function _127(Inputs,DNFs){return(
Inputs.table(DNFs)
)}
...
function _130(Inputs,laps){return(
Inputs.table(laps)
)}
...
function _lHamSprint(sprint_results,raceIds2021Set,driverMapRev){return(
sprint_results.filter(d=>raceIds2021Set.has(d.raceId)).filter(d=>d.driverId==driverMapRev.get("Lewis Hamilton")).map(d=>({'raceId':d.raceId,'points':d.points})).sort((a,b) => a.raceId - b.raceId).map(d=>({'raceId':d.raceId,'points':d.points}))
)}

function _lHamSprintMap(d3,lHamSprint){return(
d3.group(lHamSprint,
                        d=>d.raceId)
)}

function _lHam(raceResults2021,driverMapRev){return(
raceResults2021.filter(d=>d.driverId==driverMapRev.get("Lewis Hamilton")).map(d=>({'raceId':d.raceId,'points':d.points})).sort((a,b) => a.raceId - b.raceId)
)}

function _lHamMap(d3,lHam){return(
d3.group(lHam,
                   d=>d.raceId)
)}

function _hamPts(lHamMap,lHamSprintMap)
{
  let res = []
  for(let [k,v] of lHamMap){
      if(lHamSprintMap.has(k))
        res.push(v[0].points+lHamSprintMap.get(k)[0].points)
      else
        res.push(v[0].points)
  }
  return res
...
function _mVerSprint(sprint_results,raceIds2021Set,driverMapRev){return(
sprint_results.filter(d=>raceIds2021Set.has(d.raceId)).filter(d=>d.driverId==driverMapRev.get("Max Verstappen")).map(d=>({'raceId':d.raceId,'points':d.points})).sort((a,b) => a.raceId - b.raceId).map(d=>({'raceId':d.raceId,'points':d.points}))
)}

function _mVerSprintMap(d3,mVerSprint){return(
d3.group(mVerSprint,d=>d.raceId)
)}

function _mVer(raceResults2021,driverMapRev){return(
raceResults2021.filter(d=>d.driverId==driverMapRev.get("Max Verstappen")).map(d=>({'raceId':d.raceId,'points':d.points})).sort((a,b) => a.raceId - b.raceId)
)}

function _mVerMap(d3,mVer){return(
d3.group(mVer, d=>d.raceId)
)}

function _verPts(mVerMap,mVerSprintMap)
{
  let res = []
  for(let [k,v] of mVerMap){
      if(mVerSprintMap.has(k))
        res.push(v[0].points+mVerSprintMap.get(k)[0].points)
      else
        res.push(v[0].points)
  }
  return res
...
function _mVerCuSum(accumulate,verPts){return(
accumulate(verPts)
)}

function _lHamCuSum(accumulate,hamPts){return(
accumulate(hamPts)
)}
...
function _156(Inputs,q9datafinal){return(
...
function _164(Inputs,q10data){return(
...
function _175(Inputs,viz_data11){return(
...
export default function define(runtime, observer) {
  ...
  main.variable(observer()).define(["Inputs","q5_pitStopsData"], _86);
  ...
  main.variable(observer()).define(["Inputs","q6final"], _97);
  ...
  main.variable(observer("races2021")).define("races2021", ["d3","races"], _races2021);
  ...
  main.variable(observer("q7datafinal")).define("q7datafinal", ["q7data"], _q7datafinal);
  main.variable(observer("q7VizData")).define("q7VizData", ["q7data"], _q7VizData);
  main.variable(observer("const2021")).define("const2021", ["raceResults2021"], _const2021);
  main.variable(observer("q7dataconst")).define("q7dataconst", ["const2021","q5constructorMap","accumulate","raceResults2021"], _q7dataconst);
  main.variable(observer()).define(["Inputs","q7data"], _111);
  ...
  main.variable(observer("lHamSprint")).define("lHamSprint", ["sprint_results","raceIds2021Set","driverMapRev"], _lHamSprint);
  main.variable(observer("lHamSprintMap")).define("lHamSprintMap", ["d3","lHamSprint"], _lHamSprintMap);
  main.variable(observer("lHam")).define("lHam", ["raceResults2021","driverMapRev"], _lHam);
  main.variable(observer("lHamMap")).define("lHamMap", ["d3","lHam"], _lHamMap);
  main.variable(observer("hamPts")).define("hamPts", ["lHamMap","lHamSprintMap"], _hamPts);
  main.variable(observer("mVerSprint")).define("mVerSprint", ["sprint_results","raceIds2021Set","driverMapRev"], _mVerSprint);
  main.variable(observer("mVerSprintMap")).define("mVerSprintMap", ["d3","mVerSprint"], _mVerSprintMap);
  main.variable(observer("mVer")).define("mVer", ["raceResults2021","driverMapRev"], _mVer);
  main.variable(observer("mVerMap")).define("mVerMap", ["d3","mVer"], _mVerMap);
  main.variable(observer("verPts")).define("verPts", ["mVerMap","mVerSprintMap"], _verPts);
  main.variable(observer("accumulate")).define("accumulate", _accumulate);
  main.variable(observer("mVerCuSum")).define("mVerCuSum", ["accumulate","verPts"], _mVerCuSum);
  main.variable(observer("lHamCuSum")).define("lHamCuSum", ["accumulate","hamPts"], _lHamCuSum);
  ...
  main.variable(observer()).define(["Inputs","q9datafinal"], _156);
  ...
  main.variable(observer()).define(["Inputs","q10data"], _164);
  ...
  main.variable(observer()).define(["Inputs","viz_data11"], _175);
